...
## Loads the results from the GAM model fit
load("../HMM-GAMgevRes/results.RData")



postscript(paste("Figure 3.eps",sep=""),paper="special",
           width=3*2,height=5)
...
format(cbind(HMMest$loct,loc.bt.lo,
      loc.bt.up,
      HMMest$scale,scale.bt.lo,
      scale.bt.up,
      HMMest$shape,shape.bt.lo,
      shape.bt.up),digits=2)
...
sunspot <- read.csv("../MaximaData/SunspotNumber.csv")
yrs <- sunspot$Year
mth <- sunspot$Month
sub.spot <- sunspot[yrs>=1994 & yrs <= 2019,]
yrs.sub <- sub.spot$Year
mth.sub <- sub.spot$Month

range(sub.spot$spot.no)
sunspot.wk <- c()
for (i in 1:len.data){
  sunspot.wk[i] <- sum(sub.spot$spot.no[((i-1)*7+1):(i*7)])
}

sunspot.mth <- NULL
for (i in 1994:2019){
  for (j in 1:12){
    sunspot.mth <- c(sunspot.mth,sum(sub.spot$spot.no[yrs.sub==i & mth.sub == j]))
  }
}


postscript(paste("Figure 4.eps",sep=""),paper="special",
    width=12/1.15,height=9/1.15) 
par(mfrow=c(3,3))

par(mar=c(4.5, 4.5, 1.5, 3.5)) 
plot(1:length(R1wkmax),loct.all,type="l",ylim=c(0,15),
     xlab="year",ylab="GEV location",axes=F,cex.lab=1.5)
box()
axis(1,at=wksoveryr[seq(1,26,by=5)],(1995:2020)[seq(1,26,by=5)],cex.axis=1.5)
axis(2,at=c(5,10,15),c(5,10,15),cex.axis=1.5)
...
lines(1:len.data,sunspot.wk/(max(sunspot.wk))*4,col="lightgrey")
lab.at <- c(0,1000,2000)/max(sunspot.wk)*4
axis(4,at=lab.at,c(0,1000,2000),cex.axis=1.5,col="darkgrey",col.axis="darkgrey")
mtext("Sunspot number", side=4, line=2.2, cex.lab=1.5,las=0,adj=0, col="darkgrey")



plot(1:length(R1wkmax),scale.all,type="l",ylim=c(-2,9),
     xlab="year",ylab="GEV scale",axes=F,cex.lab=1.5)
box()
axis(1,at=wksoveryr[seq(1,26,by=5)],(1995:2020)[seq(1,26,by=5)],cex.axis=1.5)
axis(2,at=c(2,4,6,8),c(2,4,6,8),cex.axis=1.5)
...
lines(1:len.data,sunspot.wk/(max(sunspot.wk))*3-2,col="lightgrey")
lab.at <- c(0,1000,2000)/max(sunspot.wk)*3-2
axis(4,at=lab.at,c(0,1000,2000),cex.axis=1.5,col="darkgrey",col.axis="darkgrey")
mtext("Sunspot number", side=4, line=2.2, cex.lab=1.5,las=0,adj=0, col="darkgrey")



plot(1:length(R1wkmax),shape.all,type="l",ylim=c(-0.4,1),
     xlab="year",ylab="GEV shape",axes=F,cex.lab=1.5)
box()
axis(1,at=wksoveryr[seq(1,26,by=5)],(1995:2020)[seq(1,26,by=5)],cex.axis=1.5)
axis(2,at=c(0,0.2,0.4,0.6,0.8,1),c(0,0.2,0.4,0.6,0.8,1),cex.axis=1.5)
...
lines(1:len.data,sunspot.wk/(max(sunspot.wk))*0.4-0.4,col="lightgrey")
lab.at <- c(0,1000,2000)/max(sunspot.wk)*0.4-0.4
axis(4,at=lab.at,c(0,1000,2000),cex.axis=1.5,col="darkgrey",col.axis="darkgrey")
mtext("Sunspot number", side=4, line=2.2, cex.lab=1.5,las=0,adj=0, col="darkgrey")
...
plot(m4,select=2,rug=F,scale=F,ylim=c(-15,15),ylab="GEV location",cex.axis=1.5,
     cex.lab=1.5,axes=F)
box()
axis(1,cex.axis=1.5)
axis(2,at=seq(-5,15,5),seq(-5,15,5),cex.axis=1.5)
legend("topright","(g) GAM",bty="n",cex=1.5)

lines(1994+(1:len.mth)/12,sunspot.mth/(max(sunspot.mth))*7-15,col="lightgrey")
lab.at <- c(0,3000,6000)/max(sunspot.mth)*7-15
axis(4,at=lab.at,c(0,3000,6000),cex.axis=1.5,col="darkgrey",col.axis="darkgrey")
axis(4,at=lab.at[3],c(0,3000,6000)[3],cex.axis=1.5,col="darkgrey",col.axis="darkgrey")
mtext("Sunspot number", side=4, line=2.2, cex.lab=1.5,las=0,adj=0, col="darkgrey")
#axis(4,at=lab.at[3],6000,cex.axis=1.4,col="gray")


plot(m4,select=4,rug=F,scale=F,trans=exp,ylim=c(-1,3),ylab="GEV scale",cex.axis=1.5,
     cex.lab=1.5,axes=F)
box()
axis(1,cex.axis=1.5)
axis(2,at=seq(0,3,1),seq(0,3,1),cex.axis=1.5)
legend("topright","(h) GAM",bty="n",cex=1.5)

lines(1994+(1:len.mth)/12,sunspot.mth/(max(sunspot.mth))-1,col="lightgrey")
lab.at <- c(0,3000,6000)/max(sunspot.mth)-1
axis(4,at=lab.at,c(0,3000,6000),cex.axis=1.5,col="darkgrey",col.axis="darkgrey")
axis(4,at=lab.at[3],c(0,3000,6000)[3],cex.axis=1.5,col="darkgrey",col.axis="darkgrey")
mtext("Sunspot number", side=4, line=2.2, cex.lab=1.5,las=0,adj=0, col="darkgrey")
#axis(4,at=lab.at[3],6000,cex.axis=1.4,col="gray")
...
## return levels for solar maximum
vky <- mc$y
rl.50all[,1]
rl.100all[,1]
rl.200all[,1]
...
postscript(paste("Figure 5.eps",sep=""),paper="special",
           height=2.25*4.5/1.25,width=4*4.5/1.25) 
par(mfrow=c(2,2))
...
axis(1,at=wksoveryr[seq(1,26,by=5)],(1995:2020)[seq(1,26,by=5)],cex.axis=1.5)
axis(2,cex.axis=1.5)
axis(2,at=(c(20000)),20000,cex.axis=1.5)
...
## Bootstrap confidence interval of probability of exceeding a value at each time point conditioning on 
## the Viterbi path
...
# Estimated probability of exceedance at each time point
## Bootstrap 
B <- 10000
th.shd <- 500
pe.wk.state <- matrix(0,3,nk)
for (jj in 1:nk){
  pe.wk.fun <- function(x){
    loc.bt <- gev.param[1:(nk)]
    scale.bt <- exp(gev.param[(nk+1):(2*nk)])
    shape.bt <- gev.param[(2*nk+1):(3*nk)]
    
    pe.wk <- pevd(x, loc = loc.bt[jj], scale = scale.bt[jj], shape = shape.bt[jj], 
                     type = c("GEV"), lower.tail = TRUE)
    pe.wk
  }
  
  
  pe.wk.state[1,jj] <- pe.wk.fun(th.shd)
  ...
  for (b in 1:B){
    ...
    pe.wk.bt.s <- function(x){
      loc.bt <- gev.param.bt[1:(nk)]
      scale.bt <- exp(gev.param.bt[(nk+1):(2*nk)])
      shape.bt <- gev.param.bt[(2*nk+1):(3*nk)]
      
      rl.fun <- pevd(x, loc = loc.bt[jj], scale = scale.bt[jj], shape = shape.bt[jj], 
                       type = c("GEV"),lower.tail = TRUE)
      rl.fun 
    }
    
    pe.wk.bt[b] <- pe.wk.bt.s(th.shd)
  }
    
  pe.wk.state[2:3,jj] <- quantile(pe.wk.bt,probs = c(0.025,0.975))
  
}



pe.wk.all <- matrix(0,3,length(R1hrmax))
for (k in 1:length(R1hrmax)){
  pe.wk.all[,k] <- pe.wk.state[,vky[k]]
}

pe.yr.all <- matrix(0,3,floor(length(R1hrmax)/52))
for (j in 1:floor(length(R1hrmax)/52)){
  tem <- 1
  for (i in ((j-1)*52+1):(j*52)){
...
postscript(paste("Figure 6.eps",sep=""),paper="special",
           height=2.25*4.5/1.25/2,width=4*4.5/2.5) 
par(mar=c(4.5, 4.5, 1.5, 1)) 
plot(1:26,pe.yr.all[1,],type="l",ylim=c(min(pe.yr.all),inc*max(pe.yr.all)),
     xlab="year",ylab="Prob exceeding 500 nT/min",axes=F,cex.lab=1.5)
box()
axis(1,at=seq(1,26,4),seq(1994,2019,4),cex.axis=1.5)
axis(2,cex.axis=1.5)
lines(1:26,pe.yr.all[2,],lty="dashed",col="black")
lines(1:26,pe.yr.all[3,],lty="dashed",col="black")
...
postscript(paste("Figure 7.eps",sep=""),paper="special",
           height=2.25*4.5,width=4*4.5) 
par(mfrow=c(2,2))

par(mar=c(4.7, 4.7, 1.5, 1)) 
...
for(i in 1:nrp){
  ...
  plot(rl.meds4[,i],typ="l",xaxt="n",xlab="year",ylab="Return level (nT/min)",
       ylim=ylims,log="y",lwd=2,cex.axis=2,cex.lab=2)
  lines(rl.lows4[,i])
  lines(rl.upps4[,i])
  axis(1,at=ats,labels=labs,cex.axis=2,cex.lab=2)
  legend("topleft",paste0(letters[i]," ",labels[i]),bty="n",cex=2)
}
...
postscript(paste("Figure 8.eps",sep=""),paper="special",
           height=2.25*4.5/1.1/2,width=4*4.5/1.4) 
par(mfrow=c(1,2))

par(mar=c(4.5, 4.5, 1.5, 1)) 
plot(sunspot.wk,R1wkmax, xlab="Total sunspot number in each week",ylab="R1 (nT/min)",cex.lab=1.6,cex.axis=1.6)
legend("topleft","(a)",bty="n",cex=1.6)

plot(sunspot.wk,R1wkmax,xlab="Total sunspot number in each week",ylab="R1 (nT/min)",log="y",cex.lab=1.6,cex.axis=1.6)
legend("topleft","(b)",bty="n",cex=1.6)
dev.off()


m1 <- lm(log(R1wkmax)~sunspot.wk)
summary(m1)
